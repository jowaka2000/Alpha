                ...
                <div class="pt-10 w-11/12 md:w-10/12">
                    <div class="flex justify-between items-center">
                        <div class="flex items-center space-x-4">
                            <a href="{{ url()->previous() === route('unlocks.edit', $unlock) ? route('unlocks.index') : url()->previous() }}"
                            ...
                            <div class="text-2xl md:text-3xl text-neutral-400 font-bold pb-1">Edit</div>
                        </div>
                        @can('delete', $unlock)
            ...
            <div>
                ...
                <div class="w-full py-5 flex justify-center">
                    <div class="w-full md:w-6/12 flex px-5 pt-5 ">
                        <form action="{{ route('unlocks.edit', $unlock) }}"
                            ...
                            <div>
                                ...
                                <select name="unlock_type" id="unlock_type"
                                    ...
                                    @foreach ($unlockPrices as $key=>$amount)
                                        <option value="{{$key}}"
                                            {{ old('unlock_type') === $key || $unlock->unlock_type === $key ? 'selected' : '' }}
                                            class="bg-slate-700">{{$key}} (@ ${{$amount.' USD'}})</option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <textarea type="text" name="unlock_link" id="unlock_link" rows="3"
                                    ...
                                    placeholder="Link of your question from unlocks website">{{ old('unlock_link') ?? $unlock->unlock_link }}</textarea>
                            </div>

                            <div>
                                <label for="question" class="text-neutral-400 font-semibold">Task</label>
                                <textarea name="question" id="question" rows="5"
                                    ...
                                    placeholder="Type your question here">{{ old('question') ?? $unlock->question }}</textarea>
                                @error('question')
                                    <div class="text-sm text-red-500">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <textarea name="instructions" id="instructions" rows="3"
                                    class="w-full bg-transparent rounded text-sm text-neutral-300" placeholder="Type instrucions to be followed">{{ old('instructions') ?? $unlock->instructions }}</textarea>
                            </div>
                            ...
                            <div class="w-full">
                                ...
                                <div class="w-full space-y-2">
                                    @foreach ($files as $file)
                                        <form action="{{ route('unlock-file-delete', $file) }}" method="POST"
                                            class="w-full">
                                            @method('DELETE')
                                            @csrf
