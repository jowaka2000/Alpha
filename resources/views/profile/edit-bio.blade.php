...
@section('content')
    <div class="w-full  flex justify-center pt-5 md:pt-8">
        <div class="w-11/12 md:w-8/12 flex items-center text-neutral-400 font-semibold space-x-3">
            <a href="{{ url()->previous() }}" class="border rounded border-gray-500">
    ...
    <div class="w-full  flex justify-center py-10 h-full scroll-y-auto">
        <div class="w-11/12 md:w-7/12 flex">
            ...
            <form action="{{ route('my-profile.edit-description', $user) }}" method="POST"
                ...
                <div class="w-full space-y-6">
                    ...
                    <div class="space-y-1 shadow-lg py-3 px-2">
                        <label for="cv" class="text-sm text-neutral-400 font-medium">Upload Job Samples</label>
                        <input type="file" name="samples" name="samples[]" class="w-full text-neutral-300" multiple>
                    </div>
                            ...
                            @foreach ($subjects as $subject)
                                <div class="text-sm text-neutral-400 flex items-center gap-2">
                                    <input type="checkbox" name="subjects[]" value="{{ $subject->subject }}"
                                        id="{{ $subject->subject }}" class="rounded" {{array_search(json_decode($bio->subjects,true),$subject->subject) ? 'selected' : ''}}>
                    ...
                    <div class="flex w-full justify-end">
                        <button type="submit"
                            class="px-4 py-2 bg-green-500 text-green-900 hover:bg-green-400 rounded font-bold">Update Bio</button>
                    </div>
                </div>
                @csrf
            </form>
