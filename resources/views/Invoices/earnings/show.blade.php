                    ...
                    <div class="flex pb-3 justify-between shadow-xl">
                        <div class="flex space-x-3 w-auto">
                            <div class="flex items-center justify-center">
                                <a href=""><img src="{{ $order->user->image==null ? asset('images/user.png') : asset('images/'.$order->user->image) }}" alt="About {{ $order->user->name }}"
                                        class="rounded-full w-12 h-12 b-3"></a>
                            </div>

                            <div class="text-neutral-300 mt-2">
                                <div class="font-semibold flex items-center"> <span>{{ strtoupper($order->user->name) }}</span>
                                    <span class="hidden md:flex text-neutral-500"> ({{ $order->user->chanel }}) <span></span>
                                </div>
                                <div class="mb-3 opacity-75">
                                    @can('isMyEmployer', $order->user)
                                      My Employer
                                    @else
                                       Not My Employer
                                    @endcan
                        ...
                        <div class="px-2">
                            <a href="{{ route('report.index', $order->user) }}"
                    ...
                    <div class="flex p-3">
                        <div class="flex space-x-3">
                            <div class="space-y-2 text-neutral-400 border-r pr-4 border-neutral-500">
                                <div class="font-semibold {{ $order->user->reliable_rate == null ? 'opacity-50' : '' }}">
                                    {{ $order->user->reliable_rate == null ? 'Not Rated' : $order->user->reliable_rate . '% Reliable' }}
                                </div>

                                <div class="hidden md:flex">
                                    <div class="flex items-center">
                                        <svg aria-hidden="true"
                                            class="w-4 h-4 {{ $order->user->reliable_rate >= 20 ? 'text-yellow-400' : 'text-gray-300' }}"
                                        ...
                                        <svg aria-hidden="true"
                                            class="w-4 h-4 {{ $order->user->reliable_rate >= 40 ? 'text-yellow-400' : 'text-gray-300' }}"
                                            fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                            <title>First star</title>
                                            <path
                                                d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z">
                                        ...
                                        <svg aria-hidden="true"
                                            class="w-4 h-4 {{ $order->user->reliable_rate >= 60 ? 'text-yellow-400' : 'text-gray-300' }}"
                                            fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                            <title>First star</title>
                                            <path
                                                d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z">
                                        ...
                                        <svg aria-hidden="true"
                                            class="w-4 h-4 {{ $order->user->reliable_rate >= 80 ? 'text-yellow-400' : 'text-gray-300' }}"
                                            fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                            <title>First star</title>
                                            <path
                                                d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z">
                                        ...
                                        <svg aria-hidden="true"
                                            class="w-4 h-4 {{ $order->user->reliable_rate >= 100 ? 'text-yellow-400' : 'text-gray-300' }}"
                                            fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                            <title>First star</title>
                                            <path
                                                d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z">
                            ...
                            <div class="text-neutral-400 border-r pr-4 border-neutral-500">
                                <span
                                    class="font-semibold">{{ $order->user->orders == null ? '0' : $order->user->orders }}</span>
                                <div class="text-sm">Orders</div>
                            </div>

                            <div class="text-neutral-400 border-r pr-4 border-neutral-500">
                                <span class="font-semibold">{{$employerWriters}}</span>
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gat-0 md:gap-4 space-y-3 md:space-y-0">
                    ...
                    <span
                        class="flex text-sm items-center"><span>{{ $order->completed->created_at->format('D, M y H:i') }}</span><span
                            class="text-xs items-center text-orange-500">({{ (new \Carbon\Carbon($order->deadline))->diffForHumans() }})</span></span></span>
