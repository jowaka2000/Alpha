    ...
    <div class="flex w-full justify-center py-8">
        <div class="flex w-11/12 md:w-7/12 justify-between">
            ...
            <div class="flex items-center gap-4 border-b border-slate-600">
                <a href="{{ route('unlocks-subsription.mpesa', $plan) }}"
                    class="text-gray-300 text-lg {{ request()->routeIs('unlocks-subsription.mpesa', $plan) ? 'font-bold border-b-4 border-slate-600' : '' }}">M-PESA</a>
                <a href="{{ route('unlocks-subsription.paypal', $plan) }}"
                    class="text-gray-300 {{ request()->routeIs('unlocks-subsription.paypal', $plan) ? 'font-bold border-b-4 border-slate-600' : '' }} text-lg">PayPal</a>

            </div>

            <a class="border border-slate-600 hover:border-slate-400 p-2 rounded text-slate-400"
                href="{{ route('unlocks-subscriprion.index') }}">
        ...
        <div class="flex w-10/12 md:w-4/12 justify-center">
            ...
            <div class="w-full">
                <div class="w-full flex justify-center  text-neutral-400 font-semibold text-xl">
                    {{ $plan == 1 ? 'ORDER \'s ONE MONTH PLAN SUBSCRIPTION' : ($plan == 2 ? 'ORDER\'S  THREE MONTH PLAN SUBSCRIPTION' : ($plan == 3 ? 'ORDER\'S ONE MONTHS PLAN RENEWAL' : 'ORDER\'S THREE MONTHS PLAN RENEWAL')) }}
                </div>
                <form action="{{ route('unlocks-subsription.mpesa', $plan) }}" method="POST" class="w-full py-4 space-y-3">
                    ...
                    <div class="w-full">
                        ...
                        <input type="number" name="amount" id="amount" step="0.01" readonly
                            class="text-sm text-neutral-300 w-full bg-transparent rounded cursor-not-allowed"
                            placeholder="Amount"
                            value="{{$amount}}">
                    ...
                    <div class="w-full">
                        <button type="submit"
                            class="bg-green-500 w-full flex justify-center px-3 py-2 rounded text-slate-300 text-lg font-semibold hover:bg-green-700">Pay via MPESA</button>
                    </div>
                    @csrf
                </form>
                ...
                @if ($wallet >= $amount)
                    <div class="w-full">
                        <a href="{{route('unlocks-subscription.pay-from-wallet',$plan)}}"
