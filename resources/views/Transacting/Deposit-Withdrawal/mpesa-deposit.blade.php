    ...
    <div class="flex w-full justify-center py-8">
        <div class="flex w-11/12 md:w-7/12 justify-between">
            ...
            <div class="flex items-center gap-4 border-b border-slate-600">
                <a href="{{route('deposit.mpesa',$user)}}" class="text-gray-300 text-lg {{request()->routeIs('deposit.mpesa',$user) ? 'font-bold border-b-4 border-slate-600' : ''}}">M-PESA</a>
                <a href="{{route('deposit.paypal',$user)}}" class="text-gray-300 {{request()->routeIs('deposit.paypal',$user) ? 'font-bold border-b-4 border-slate-600' : ''}} text-lg">PayPal</a>

            </div>

            <a class="border border-slate-600 hover:border-slate-400 p-2 rounded text-slate-400"
                href="{{ url()->previous()===route('deposit.mpesa',$user) || url()->previous()===route('deposit.paypal',$user) || url()->previous()===route('depost.mpesa.wait',$user) ? route('orders.index') : url()->previous() }}">
    ...
    <div class="flex w-full justify-center py-8">
        <div class="flex w-7/12 md:w-3/12 justify-center">
                ...
                <div class="w-full">
                    ...
                    <form action="{{route('deposit.mpesa',$user)}}" method="POST" class="w-full py-4 space-y-3">
                        <div class="w-full flex justify-center text-neutral-300">
                            Deposit to your wallet using M-pesa
                        </div>
                        ...
                        <div class="w-full">
                            <label for="amount" class="text-neutral-300">Amount to Deposit (US $)</label>
                            ...
                            @error('amount')
                                <div class="text-red-500 text-xs">{{ $message }}</div>
                            @enderror
                            <div class="text-sm text-orange-600">Minimum deposit is US $1 (Ksh. {{$rate}})</div>
                        </div>
